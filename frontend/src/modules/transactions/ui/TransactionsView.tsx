...
export const TransactionsView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!sentinel || activeTab !== 'normal') return;
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <div className="transactions-tabs" role="tablist" aria-label="Payment type">
          <button
            type="button"
            role="tab"
            aria-selected={activeTab === 'normal'}
            className={`transactions-tab ${activeTab === 'normal' ? 'transactions-tab-active' : ''}`}
            onClick={() => setActiveTab('normal')}
          >
            {strings.normalPaymentsTab}
          </button>
          <button
            type="button"
            role="tab"
            aria-selected={activeTab === 'recurring'}
            className={`transactions-tab ${activeTab === 'recurring' ? 'transactions-tab-active' : ''}`}
            onClick={() => setActiveTab('recurring')}
          >
            <Repeat size={14} />
            {strings.recurringPaymentsTab}
          </button>
        </div>
