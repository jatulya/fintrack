...
import { Landmark, Layers, PiggyBank, Repeat, TrendingUp, Wallet } from 'lucide-react';
...
export const AssetAllocationCard: React.FC<AssetAllocationCardProps> = ({
  ...
  return (
    <div className="savings-panel mb-6">
      ...
      {isLoading ? (
        <p className="text-body-muted m-0">Loading where the money naps...</p>
      ) : items.length === 0 ? (
        <p className="text-body-muted m-0">{strings.savingsAssetLocationEmpty}</p>
      ) : (
